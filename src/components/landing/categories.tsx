      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2.5">
          {CATEGORIES.map((category, index) => (
            ...
            >
              <Link href={`/search?type=${category.typeValue}`} className="group block relative rounded-xl overflow-hidden aspect-[4/3]">
                <img src={category.image} alt={category.name} className="absolute inset-0 h-full w-full object-cover transition-transform duration-500 group-hover:scale-105" loading="lazy" />
